...
void ECB_Encryption(char* inputfile, char* outputfile, u32 W[]) {
	...
	fseek(rfp, 0, SEEK_END);	//파일의 끝으로 포인터 이동
	DataLen = ftell(rfp);	//inputfile 의 크기 (바이트 수)
	fseek(rfp, 0, SEEK_SET);

	r = DataLen % 16;	//끝 바이트가 몇개 인지
	r = 16 - r;	//padding 을 얼마나 해야 하는지 PKCS#7
	
	inputbuf = calloc(DataLen + r, sizeof(u8));		// inputfile 크기 + padding 해야 하는 바이트 수
	outputbuf = calloc(DataLen + r, sizeof(u8));
	fread(inputbuf, 1, DataLen, rfp);

	fclose(rfp);
	memset(inputbuf + DataLen, r, r);	//r 로 r 만큼 채우기
