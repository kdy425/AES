//마지막 라운드 키를 이용해 각 라운드 키 출력
#include <stdio.h>
#include <stdint.h>
#include <string.h>

#define Nk 4  // 키의 워드 수
#define Nr 10 // 라운드 수
#define Nb 4  // 블록 크기 (워드)

// S-box 테이블
static const uint8_t sbox[256] = {
    
};

// Rcon 테이블
static const uint8_t Rcon[10] = {
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

void SubWord(uint8_t* word) {
    for (int i = 0; i < 4; i++) {
        word[i] = sbox[word[i]];
    }
}


void RotWord(uint8_t* word) {
    uint8_t temp = word[0];
    word[0] = word[1];
    word[1] = word[2];
    word[2] = word[3];
    word[3] = temp;
}

void InverseKeyExpansion(uint8_t* lastRoundKey, uint8_t* roundKeys) {
    // 마지막 라운드 키 복사
    memcpy(roundKeys + Nr * 16, lastRoundKey, 16);

    // 역방향으로 계산
    for (int round = Nr; round > 0; round--) {
        uint8_t* current = roundKeys + round * 16;
        uint8_t* prev = roundKeys + (round - 1) * 16;

        // 워드 4개를 역순으로 계산
        for (int i = 3; i >= 0; i--) {
            if (i == 0) {
                // 첫 번째 워드의 경우
                uint8_t temp[4];
                memcpy(temp, prev + 12, 4);  // 이전 라운드의 마지막 워드
                RotWord(temp);
                SubWord(temp);
                temp[0] ^= Rcon[round - 1];

                for (int j = 0; j < 4; j++) {
                    prev[j] = current[j] ^ temp[j];
                }
            }
            else {
                // 나머지 워드의 경우
                for (int j = 0; j < 4; j++) {
                    prev[i * 4 + j] = current[i * 4 + j] ^ current[(i - 1) * 4 + j];
                }
            }
        }
    }
}

int main() {
    uint8_t lastRoundKey[16] = {0xF6, 0x4E, 0x80, 0x0B, 0xD1, 0xF9, 0xF0, 0xA5, 0x23, 0xD5, 0x4C, 0x24, 0xAD, 0x02, 0x97, 0xFD};

    uint8_t roundKeys[176];  // 16 bytes * 11 rounds

    InverseKeyExpansion(lastRoundKey, roundKeys);

    // 모든 라운드 키 출력
    for (int round = 0; round <= Nr; round++) {
        printf("Round %2d: ", round);
        for (int j = 0; j < 16; j++) {
            printf("%02X ", roundKeys[round * 16 + j]);
        }
        printf("\n");
    }

    return 0;
}
